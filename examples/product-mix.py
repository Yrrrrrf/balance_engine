...
def main():
    ...
    z = {
        (i, j): pulp.LpVariable(f"z_{i}_{j}", lowBound=0)
        for i in raw_materials
        for j in products
    }

    # y[j] = total amount of product j produced (tons)
    y = {j: pulp.LpVariable(f"y_{j}", lowBound=0, upBound=demand[j]) for j in products}

    # Objective function: maximize total profit
    revenue = pulp.lpSum([selling_price[j] * y[j] for j in products])
    material_costs = pulp.lpSum(
        [material_cost[i] * z[i, j] for i in raw_materials for j in products]
    )
    ...
    for i in raw_materials:
        model += (
            pulp.lpSum([z[i, j] for j in products]) <= max_available[i],
            f"Material_Availability_{i}",
        )

    # 2. Mass balance constraints (sum of raw materials = product quantity)
    for j in products:
        model += (
            pulp.lpSum([z[i, j] for i in raw_materials]) == y[j],
            f"Mass_Balance_{j}",
        )

    # 3. Octane requirements
    for j in products:
        weighted_octane = pulp.lpSum(
            [octane_number[i] * z[i, j] for i in raw_materials]
        )
        model += (
            weighted_octane >= octane_requirement[j] * y[j],
            f"Octane_Requirement_{j}",
        )
    ...
    if model.status == pulp.LpStatusOptimal:
        ...
        for i in raw_materials:
            ...
            print(
                f"{i}: {total_used:.2f} tons ({utilization:.1f}% of available {max_available[i]} tons)"
            )
        ...
        for j in products:
            if y[j].value() > 0.001:  # Only print if product is produced
                total_product = y[j].value()
                achieved_octane = (
                    sum(octane_number[i] * z[i, j].value() for i in raw_materials)
                    / total_product
                )
                print(
